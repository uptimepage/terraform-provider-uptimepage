...
~> The dashboard's one-tap Telegram channels (kind `telegram_app`, linked by
pressing **Start** in the chat) are created and owned by that flow — the API
rejects the kind in request bodies, so this resource cannot create, import,
or adopt them. Use `type = "telegram"` with your own bot token instead.
