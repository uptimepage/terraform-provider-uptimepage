...
| Name | Kind | Notes |
|------|------|-------|
| `uptimepage_target` | resource | Monitors. Check types: `http`, `tcp`, `ping`, `heartbeat`, `tls_cert`, `domain_expiry`, `dns`, `flow`. The type is fixed after creation: changing it replaces the monitor. |
| `uptimepage_notification_channel` | resource | `webhook`, `slack`, `telegram`, `discord`, `msteams`, `google_chat`, `mattermost`, `email`, `pagerduty`, `ntfy`, `gotify`, `pushover`, `whatsapp`, `sms`. |
